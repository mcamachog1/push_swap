...
* At the start, the stack *a* contains a random number of **unique** positive and/or negative integers
* Stack *b* is initally empty
* The goal is to sort the numbers in stack *a* in ascending order (starting from the top of the stack)

The program calculates and displays the shortest sequence of instructions required to achieve the sort.
...
* If there are no arguments, the program return the prompt
* If the argument is an empty string return 'Error\n'
* If the argument is a string just with spaces return 'Error\n' 
...
1. **Min-Selection:** Identifies the smallest element, rotates it to the top of *a*, and pushes it to *b*
...
1. Memory Safety: Since the number of integers is known at the start, I allocate memory once, reducing the risk of leaks associated with frequent malloc calls.
...
```
## Bonus (checker)
The bonus part consists of a checker program. Its goal is to verify if the instructions generated by push_swap actually sort the stack.

### Input Handling: 
The program uses the function get_next_line, that was include in my libft, to read instructions from the standard input.

### Functionality:
It executes the received instructions on the initial stack.

* It displays "OK" if the stack *a* is sorted and stack *b* is empty.
* It displays "KO" if the stack *a* remains unsorted.
* It displays "Error" for invalid instructions or arguments.
...
* Error: 
The checker must display "Error" on stderr for invalid input
    * Non-numeric arguments: `./checker 3 2 one 0`
    * Duplicate numbers: `./checker 1 2 1`
    * Invalid instructions: `echo "abc" | ./checker 1 2 3`

* Pipe test:
`ARG="4 67 3 87 23"; ./push_swap $ARG | ./checker $ARG`

* No arguments (must show the prompt):
`./checker`